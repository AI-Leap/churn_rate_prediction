...
def test_eda(perform_eda):
    ...
    try:
      assert len(files) == 5
      assert files == ['customer_age.png', 'marital_status.png', 'heat_map.png', 'total_trans_ct.png', 'churn_histogram.png']
      logging.info("Testing perform_eda: SUCCESS")
    except AssertionError as err:
      logging.error("Testing perform_eda: There are not required plot images in directory")
...
def test_encoder_helper(encoder_helper):
    ...
    try:
        ...
        assert sorted(last_five_columns) == sorted(['Gender_Churn', 'Education_Level_Churn',
          'Marital_Status_Churn', 'Income_Category_Churn', 'Card_Category_Churn'])
        logging.info("Testing encoder_helper: SUCCESS")
    except AssertionError as err:
        logging.error("Testing encoder_helper: The number of categorical columns is not equal to the number of categorical columns")
        raise err
...
def test_perform_feature_engineering(perform_feature_engineering):
    ...
    '''
    [X_train, X_test, y_train, y_test] = cls.perform_feature_engineering(perform_feature_engineering, 'Churn')
    ...
    try:
        ...
        logging.error("Testing perform_feature_engineering: The number of rows in the training and testing data is not equal")
        raise err
